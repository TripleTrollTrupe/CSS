...
abstract class BookshelfUI extends IBookshelfUI {
	 ...
	 * @throws Exception 
	 */
	private JTree createTree() throws Exception {
	...
	private void internalAddShelfNode(String name, JPopupMenu contextMenu, 
		...
		// TODO: Remover no futuro quando for possivel criar e apagar smart shelves
		contextMenu = !name.equals(AppProperties.INSTANCE.RECENTLY_BORROWED_SHELF_NAME) ? contextMenu : null;
		DefaultMutableTreeNode shelf = new DefaultMutableTreeNode(new TreeNodeUserData(name, contextMenu, action));
		parent.add(shelf);
	...
	public void showEMedia (String target, Iterable<EMedium> eMedia) {
		...
		for (EMedium e : eMedia) {
			internalAddToEMediaPanel (target, e, contextMenu);
		}
		SwingUtilities.updateComponentTreeUI(eMediaPanel);
	}

	@Override
	public void returnEMediumFromPanel (EMedium eMedium) {
	...
	public void addToEMediaPanel (String target, EMedium eMedia) {
		internalAddToEMediaPanel(target, eMedia, getContextMenuForTarget(target));
		SwingUtilities.updateComponentTreeUI(eMediaPanel);
	}
	...
	private JPopupMenu getContextMenuForTarget(String target) {
		...
		if (target == AppProperties.INSTANCE.LIBRARY_NAME) {
			...
			contextMenu = createEMediaRentalShelfContextMenu(target);
	...
	private void internalAddToEMediaPanel(String target, EMedium eMedium, JPopupMenu contextMenu) {
		try {
			...
			thumbnailLabel.setEnabled(target.equals(AppProperties.INSTANCE.LIBRARY_NAME) || !eMedium.isExpired());
		} catch (IOException e) {
			e.printStackTrace();
	...
	private JPopupMenu createEMediaRentalShelfContextMenu (String shelfName) {
		JPopupMenu contextMenu = new JPopupMenu ();
		...
		contextMenu.add(createMenuItem("Return rental", bookshelfEMediaController.returnRental(shelfName)));
		return contextMenu;
	}
