...
class LibraryDropTarget extends EMediumDropTarget {
	...
	public LibraryDropTarget(JFrame frame, JTree tree, JPanel documentsPanel, BookshelfUIDelegate uiDelegate) {
    ...
    public void dragEnter(DropTargetDragEvent event) {
		if (isLibrarySelected(event) && isSupportedFlavor(event))
			event.acceptDrag(DnDConstants.ACTION_LINK);
		else
    		event.rejectDrag();
    }

	private boolean isSupportedFlavor(DropTargetDragEvent event) {
					...
					protected Void doInBackground() throws Exception {
						for (File f : files) {
							...
							if (dialogData.didUserAccept()) {
									boolean resultAdd = uiDelegate.addEMediumLibrary(dialogData.data); 
									if (!resultAdd)
